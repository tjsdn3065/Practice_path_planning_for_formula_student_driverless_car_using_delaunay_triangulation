...
//  - 폐곡(closed)과 비폐곡(open)을 모두 지원하도록 정리
//  - emit_closed_duplicate=true  → 폐곡 모드
//  - emit_closed_duplicate=false → 비폐곡(첫 바퀴 등 부분맵) 모드
// ---------------------------------------------------------------------------
...
namespace cfg
{
    struct Config
    {
        ...
        bool   use_dynamic_samples = true; // true면 콘 개수 기준으로 자동 설정
        double sample_factor_n     = 1.1;  // n: 샘플 수 = n * (#inner + #outer)
        int    samples_min         = 10;   // 과도한 희소화 방지
        int    samples_max         = 250; // 과도한 과샘플 방지(너무 크면 raceline≈centerline)
        ...
        bool enable_boundary_len_filter = true; // 길이 필터 적용 여부 (처음엔 false로 두고 관찰)
        double boundary_edge_len_scale = 2.0;    // 길이 컷오프 = scale * 중앙값
        double boundary_edge_abs_max = 6.0;      // 절대 상한(미사용하려면 아주 크게)

        // 차량 폭/마진 (코리도 가드)
        double veh_width_m = 1.0;
        double safety_margin_m = 0.05;

        // 출력 옵션: 폐루프 시 마지막 샘플 = 첫 샘플 복제 여부 (open/closed 모드 스위치)
...
namespace centerline
{
    ...
    struct BoundaryEdgeInfo
    {
        int u, v;
        double len;
        bool is_hull;   // 인접 삼각형 1개면 외곽(hull) 엣지
        geom::Vec2 mid; // 중점
    };
                                ...
                                const vector<int> &labels)
    {
        ...
        for (const auto &kv : M)
        {
            ...
            if (labels[u] == labels[v])
                continue; // inner↔outer 가 아닌 엣지 배제

            bool hull = (kv.second.size() == 1); // 인접 삼각형 수로 hull 판정
    ...
    // --------- 자연 3차 스플라인 + 균일 재샘플 (폐곡 안정화 위해 패딩 가능) ---------
        ...
        double &s0_out, double &L_out)
    {
        ...
        for (int i = 0; i < paddingK; ++i)
            P.push_back(ordered[N - paddingK + i]); // tail -> head
        for (const auto &q : ordered)
            P.push_back(q);
        for (int i = 0; i < paddingK; ++i)
            P.push_back(ordered[i]); // head -> tail
...
namespace raceline_min_curv
{
    ...
    // (outer 루프 한 번당) Pbase, n, h에서 A1/A2/N0/W를 "고정" 계산 (open/closed 공용)
    ...
    // --- "고정된" 선형화 계수를 쓰는 비용/그래디언트 (open/closed 분기 사용) ---
    struct CostGrad
    {
        double J;
    ...
    // 코리도 폭 → α 박스제약 계산 (좌/우 레이캐스트 + 차량폭/마진 가드)
                                                 ...
                                                 bool closed) // ← open/closed 모드
    {
        ...
        for (int outer = 0; outer < C.max_outer_iters; ++outer)
        {
            ...
            // 선형화 고정 상태에서 projected step (Armijo)
            for (int it = 0; it < C.max_inner_iters; ++it)
            {
                ...
                while (bt < 20)
                {
                    ...
                    for (int i = 0; i < N; ++i)
                    {
                        ...
                        a_new[i] = std::min(hi[i], std::max(lo[i], ai)); // 박스 투영
                    }
            ...
            // 경로 업데이트 & 누적량 기록, 법선/제약 재계산 (relinearize)
            for (int i = 0; i < N; ++i)
            {
...
// enforce_constraints=false 이면 제약 삽입/복구를 생략 (Open 데이터용)
                                       ...
                                       bool enforce_constraints = true)
{
    ...
    // Open(비폐곡/부분맵) → Bowyer–Watson만 수행
    ...
    auto pushRing = [&](int base, int n)
    {
        ...
        {
            int j = (i + 1) % n;
            cons.push_back({base + i, base + j, 0});
        }
    };
    pushRing(0, nIn);
    pushRing(nIn, nOut);
...
// P에서 법선 dir로 링(또는 폴리라인) 세그먼트와의 최단 양의 교차거리
...
// 한 점 P와 법선 n에 대해 inner/outer까지의 거리(d_inner, d_outer) (open/closed 공용)
...
int main(int argc, char **argv)
{
    ...
    // 모드: emit_closed_duplicate → 폐곡(closed) 여부
    const bool closed_mode = C.emit_closed_duplicate;

    // 동적 샘플 수 계산
    if (C.use_dynamic_samples)
    {
        int total_cones = (int)inner.size() + (int)outer.size();
        int dyn = (int)std::llround(C.sample_factor_n * std::max(0, total_cones));

        // 최소/최대 가드 + 안전 하한(스플라인/최적화가 의미 있으려면 어느 정도 포인트 필요)
        dyn = std::max(dyn, C.samples_min);
        if (C.samples_max > 0)
            dyn = std::min(dyn, C.samples_max);
        dyn = std::max(dyn, 4); // 절대 하한

        if (C.verbose)
        {
            cerr << "[samples] dynamic=" << dyn
                 << "  (n=" << C.sample_factor_n
                 << ", cones=" << total_cones << ")\n";
        }
        C.samples = dyn;
    ...
    auto cdt = buildCDT_withRecovery(inner, outer, /*enforce_constraints=*/closed_mode);
    if (C.verbose)
    {
        if (closed_mode)
        {
        ...
        {
            cerr << "[CDT] open-mode triangulation (no constraints), total points="
                 << cdt.all.size() << ", faces=" << cdt.tris.size() << "\n";
        }
    }
    ...
    {
        const auto &A = cdt.all[t.a];
        const auto &B = cdt.all[t.b];
        const auto &Cc = cdt.all[t.c];
    ...
    // [4] 트랙 영역 클리핑 (+ 품질 필터) — open이면 생략
    std::vector<delaunay::Tri> faces_kept, faces_drop;
    if (closed_mode)
    {
        auto innerEtmp = clip::ringEdges(inner);
        auto outerEtmp = clip::ringEdges(outer);
        for (const auto &t : cdt.tris)
        {
            const auto &A = cdt.all[t.a];
            const auto &B = cdt.all[t.b];
            const auto &C3 = cdt.all[t.c];
            if (clip::triangleKeep(A, B, C3, inner, outer, innerEtmp, outerEtmp, medEdge))
    ...
    // [5] 경계 엣지(라벨 다름) 길이 수집 + CSV 덤프 + (옵션)길이 필터 + 중점 추출
    ...
    double Lmin = edge_lengths.front();
    double Lmed = q(0.5);
    double Lp95 = q(0.95);
    double Lmax = edge_lengths.back();

    if (cfg::get().dump_boundary_edge_lengths)
    {
        // CSV: i,j,xi,yi,xj,yj,len,is_hull,mid_x,mid_y
        std::ofstream fb(base + "_boundary_edges_len.csv");
        if (fb)
        {
            fb.setf(std::ios::fixed);
            fb.precision(9);
            fb << "i,j,xi,yi,xj,yj,len,is_hull,mid_x,mid_y\n";
            for (auto &e : binfo)
            {
                const auto &A = cdt.all[e.u];
                const auto &B = cdt.all[e.v];
                fb << e.u << "," << e.v << ","
                   << A.x << "," << A.y << ","
                   << B.x << "," << B.y << ","
                   << e.len << "," << (e.is_hull ? 1 : 0) << ","
        ...
        {
            cerr << "[WARN] cannot write boundary_edges_len.csv\n";
        }
        cerr << std::setprecision(6)
             << "[boundary] count=" << binfo.size()
             << "  Lmin=" << Lmin
             << "  Lmed=" << Lmed
             << "  L95=" << Lp95
             << "  Lmax=" << Lmax << "\n";
    ...
    // (옵션) 길이 필터 적용: 오픈 데이터에서 긴 hull-bridge 제거에 효과적
    ...
    if (mids.size() < 2)
    {
        cerr << "[ERR] not enough midpoints after length filter ("
             << mids.size() << "), try disabling filter or increasing threshold.\n";
    ...
    auto center = centerline::splineUniformClosed_EXPORT_CONTEXT(
        ordered, C.samples, paddingK, /*close_loop=*/closed_mode,
        spx, spy, s0, L);
    ...
    // [8] 기하량 + 폭(width) CSV
    auto innerE = closed_mode ? clip::ringEdges(inner) : clip::ringEdgesPolyline(inner);
    auto outerE = closed_mode ? clip::ringEdges(outer) : clip::ringEdgesPolyline(outer);

    {
        ...
        // 첫 샘플 저장(폐곡이면 마지막에 복제)
        double x0 = 0, y0 = 0, hd0 = 0, k0 = 0, din0 = 0, dout0 = 0, w0 = 0;

        const int N = (int)center.size();
        for (int k = 0; k < N; ++k)
        {
            // s-축 균일 (center는 open일 땐 중복 없음, closed일 땐 마지막=첫 점)
            int denomN = closed_mode ? (C.samples) : std::max(1, C.samples);
    ...
    // [9] 최소 곡률 raceline (open/closed 공용)
    {
        // 최적화 입력 경로: closed면 마지막 중복점 제거, open이면 그대로
        ...
        // raceline with geom
        {
            ...
            for (int k = 0; k < Nrl; ++k)
            {
                double si = s0 + L * (double(k) / double(Nrl)); // Nrl분할 균일
                double si_rel = si - s0;
